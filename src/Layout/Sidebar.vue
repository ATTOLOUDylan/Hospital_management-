...
const loadComplaintsCount = () => {
  const saved = localStorage.getItem("complaints");
  allComplaints.value = saved ? JSON.parse(saved) : [];
};

// Compte uniquement les plaintes avec le statut "En attente"
const pendingCount = computed(() => {
  return allComplaints.value.filter(c => c.status === 'En attente').length;
});
...
onMounted(() => {
  ...
  // Optionnel : Rafraîchir le compte régulièrement si l'admin reste sur la page
  setInterval(loadComplaintsCount, 5000);
});
    ...
    <nav class="flex-1 px-4 space-y-4 overflow-y-auto custom-scrollbar">
      ...
      >
        ...
        >
          ...
          <span 
            v-if="pendingCount > 0"
            class="flex items-center justify-center min-w-[20px] h-5 px-1.5 bg-rose-500 text-white text-[10px] font-black rounded-full shadow-sm shadow-rose-500/40 animate-pulse"
          >
            {{ pendingCount }}
          </span>
        </div>
